class PaypalAccount < ActiveRecord::Base
    ...
    def capture(payment)
        ...
        # MyCharmies HACK
        Rails.logger.debug("\e[1;33mMyCharmies:\e[0m  In capture, setting paypal currency to order country_code #{payment.order.country_code}")
        case payment.order.country_code
            when "AU"
                currency = "AUD"
            else
                currency = "USD"
